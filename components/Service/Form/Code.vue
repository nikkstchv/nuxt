...
export default {
  ...
  data () {
    return {
      formId: 'form_serviceMaintenance',
      isLoading: false,
      error: null,
      timer: 120,
    }
  ...
  methods: {
    ...
    async onComplete (code) {
      ...
      const res = await this.fetchClient(routes.timeslotsValidate(), options)

      if (res?.booked) {
        this.$emit('success')
        const routeKeyDefault = this.isAsc ? 'service_new' : ''
        const formData = {
          email: this.userEmail,
          routeKey: this.routeKey || routeKeyDefault,
        ...
        if (this.isIframe) {
          ...
          const iframeData = {
            ...
            title: `Заявка на сервис iFrame ${dateNow}, ${timeNow}`,
          }
    ...
    async resendCode () {
      ...
      await this.fetchClient(routes.timeslotsRegister(), options)
    },
